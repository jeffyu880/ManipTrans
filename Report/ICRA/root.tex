...
demonstrations that vary in starting configuration, approach direction and grasp, on
top of a frozen hand-motion imitator, so that the policy learns the manipulation skill
rather than a specific trajectory. To collect these demonstrations and to drive the
...
a longer-horizon task with a bimanual handover.
...
Aggregated over all five distances, demonstration distance itself does not correlate
with teleoperation failure rate; performance is determined by task phase. The residual
policy drops from 50/50 at the reach phase to 30/50 by seating, indicating that aligning
the fingers to the cap radius during grasping, and the cap's descent point with the
bottle opening during seating, are the two dominant bottlenecks rather than any
degradation from operator distance or trajectory novelty.
...
physically and in simulation (Fig.~\ref{fig:capscaling}). A thinner physical cap
...
enabled raises the mean from $0.56$ to $0.60$, but its effect on \emph{invariance} is
