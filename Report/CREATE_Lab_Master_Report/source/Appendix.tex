...
\begin{itemize}
  ...
  \item \textbf{Keypoint noise} emulates hand-pose-estimator and motion-capture error. It is
        disabled for the bottle-capping runs ($\sigma = 0$) and set to $\sigma = 0.5$\,cm for the
        brush-flipping runs, with the finger failure thresholds loosened by a matching factor to
        compensate.
\end{itemize}

\paragraph{Constrained sampling for the RH-about-LH-object rotation.}
The rejection criterion is applied to a single scalar component rather than to the full
displacement. Writing $u = p_0 - c_0$ for the initial lever arm, the rotation displaces the start
pose by $(R - I)u$, whose magnitude is $2\lvert\sin(\theta/2)\rvert\,\lVert u_{xy}\rVert$, with
$u_{xy}$ the component of $u$ perpendicular to the rotation axis. What is bounded in practice is
its world-$X$ component,
%
\begin{equation}
...
\end{equation}
%
which is likewise linear in the initial lever arm. A single scene-wide angle therefore displaces a
...
What the augmentation buys is invariance rather than raw success rate. The mean over all folds and
distances moves only from $0.56$ to $0.60$, a gain of four points. The distance marginals, however,
change substantially: unaugmented they run $0.50$, $0.42$, $0.52$, $0.64$, $0.70$ from the farthest
reach to the nearest, a spread of $28$ points that broadly tracks proximity, whereas with object yaw
they lie in $0.54$--$0.63$, a spread of $9$ with no remaining trend. Broken down by distance the
augmentation is worth $+0.21$ and $+0.11$ at the second and third reaches and $-0.10$ and $-0.07$ at
the two nearest: it lifts the far half of the workspace and gives up a little at the near half,
which is what removes the distance dependence. That dependence is the one
\Cref{sec:related-teleop} identifies as the obstacle to handling unseen operator trajectories, so
flattening it matters more for teleoperation than the four-point mean would suggest.

Rotating the object about its own vertical axis breaks the demonstrated alignment between the grasp
and the object frame, so the policy cannot rely on the cap being presented at the yaw at which it
was recorded --- the burner is never replaced on the table at the same orientation twice.

Two caveats temper this. First, the improvement is not widespread: only $13$ of the $25$ cells
improve, and three of the five folds. Second, fold-level variance increases rather than falls, the
spread of fold means widening from $0.25$ ($0.41$--$0.66$) to $0.44$ ($0.35$--$0.79$). Fold~2 in
particular falls from $0.50$ to $0.35$, its farthest-reach cell dropping from $0.51$ to $0.01$, the
largest single change in either direction. Which demonstrations fall into a given split therefore
still dominates that split's outcome, and ten demonstrations per fold are too few for individual
fold means to be read on their own.
