...
demonstration intent. Table~\ref{tab:obs-space} lists the layout for the Inspire hand ($n_d = 12$
...
\emph{what serves as the pivot}; they are summarized in Table~\ref{tab:augmentations}.
...
further later, which is what leaves the augmentation an effect.

\paragraph{Random action masking}
Following~\cite{teledexter}, a randomly chosen subset of joints is periodically frozen at its previously commanded target, so that every degree of freedom responds on schedule. This is the only augmentation acting on the \emph{action} channel rather than on the
...
throughout rather than drifting annealed over $T_{\text{ramp}}$ control
steps,
%
\begin{equation}
