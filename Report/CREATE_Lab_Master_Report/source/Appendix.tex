...
\begin{table}[H]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Keypoint noise $\sigma$$^{\ddagger}$      & $0$ / $0.5$\,cm \\
    Failure-threshold noise compensation      & $1.5$ \\
    \midrule
    ...
    \bottomrule
  \end{tabular}
  \caption{Hyperparameters of the residual policy. Reward weights are per hand and are those
  applied in the residual stage; the imitator is used as released by ManipTrans and is not
  retuned. $^{\dagger}$Object terms are additionally scaled by that hand's share of the object,
  which is $1$ for one object per hand and splits to sum to $1$ when both hands act on the same
  object. The contact term's weight ramps linearly from $0$ at a reference fingertip--object
  distance of $3$\,cm to $1$ at $2$\,cm.
  $^{\ddagger}$Where two values are given, they are the bottle-capping and brush-flipping
  settings respectively; the keypoint-noise augmentation is disabled for the capping runs.}
