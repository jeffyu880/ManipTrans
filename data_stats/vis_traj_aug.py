...
Figure 1: Wrist positions  — left=LH, right=RH
Figure 2: Object positions — left=LH object, right=RH object

Usage (from ManipTrans root):
    python data_stats/vis_traj_aug.py --data_idx 3b1e6@12 --n_aug 5 --out aug_vis.png
"""
import argparse
import sys
import numpy as np
...
def build_mujoco2gym(device="cpu"):
    _table_surface_z = 0.4 + 0.015  # table_pos.z + table_half_height (matches eval_score.py)
    T = np.eye(4)
...
def plot_3d_traj(ax, pos, label, color, is_original=False):
    ...
    ax.scatter(x[0], y[0], z[0], color=c, s=30)  # mark start
...
def main():
    ...
    R90 = torch.tensor([[cs, -sn, 0.], [sn, cs, 0.], [0., 0., 1.]], dtype=torch.float32)
    ...
    transforms = [(R90, t_zero, center)] * args.n_aug
    ...
    for i, (R, t, c) in enumerate(transforms):
        ...
        print(f"  aug {i+1}: rot={angle_deg:+.2f}°  Δx={t[0].item()*100:+.1f}cm  Δy={t[1].item()*100:+.1f}cm  center={c.tolist()}")
    print()
    ...
    print("First wrist position (RH):")
    print(f"  original:  {data_rh['wrist_pos'][0].tolist()}")
    for i, aug in enumerate(aug_rh):
        print(f"  aug {i+1}:     {aug['wrist_pos'][0].tolist()}")

    print("First wrist position (LH):")
    print(f"  original:  {data_lh['wrist_pos'][0].tolist()}")
    for i, aug in enumerate(aug_lh):
        print(f"  aug {i+1}:     {aug['wrist_pos'][0].tolist()}")

    print("First object position (RH):")
    print(f"  original:  {data_rh['obj_trajectory'][0, :3, 3].tolist()}")
    for i, aug in enumerate(aug_rh):
        print(f"  aug {i+1}:     {aug['obj_trajectory'][0, :3, 3].tolist()}")
    print()
    ...
    fig1.suptitle(f"Wrist trajectories — {args.data_idx}", fontsize=13)

    plot_3d_traj(ax_lw, data_lh["wrist_pos"], "original", "black", is_original=True)
    for i, aug in enumerate(aug_lh):
        plot_3d_traj(ax_lw, aug["wrist_pos"], f"aug {i+1}", colors[i])
    ax_lw.set_title("Left hand wrist")
    ax_lw.set_xlabel("x"); ax_lw.set_ylabel("y"); ax_lw.set_zlabel("z")
    ax_lw.legend(fontsize=7)

    plot_3d_traj(ax_rw, data_rh["wrist_pos"], "original", "black", is_original=True)
    for i, aug in enumerate(aug_rh):
        plot_3d_traj(ax_rw, aug["wrist_pos"], f"aug {i+1}", colors[i])
    ...
    plot_3d_traj(ax_lo, data_lh["obj_trajectory"][:, :3, 3], "original", "black", is_original=True)
    for i, aug in enumerate(aug_lh):
        plot_3d_traj(ax_lo, aug["obj_trajectory"][:, :3, 3], f"aug {i+1}", colors[i])
    ax_lo.set_title("Left hand object")
    ax_lo.set_xlabel("x"); ax_lo.set_ylabel("y"); ax_lo.set_zlabel("z")
    ax_lo.legend(fontsize=7)

    plot_3d_traj(ax_ro, data_rh["obj_trajectory"][:, :3, 3], "original", "black", is_original=True)
    for i, aug in enumerate(aug_rh):
        plot_3d_traj(ax_ro, aug["obj_trajectory"][:, :3, 3], f"aug {i+1}", colors[i])
