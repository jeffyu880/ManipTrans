...
| `subgoalFailTolerance` | `3.5` | `η_fail`: cumulative out-of-tolerance frames allowed per unit of `Δk` since the last hit, before the episode ends |
...
| `subgoalCrossTrajFailBudget` | `175` | Flat `n_fail` frames for the transition after a switch, in place of `η_fail · Δk`; the 15 cm object bail is suspended over it |
